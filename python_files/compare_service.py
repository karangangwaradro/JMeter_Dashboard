#!/usr/bin/env python3
"""
compare_service.py — Modern Run Comparison Service for PerfPilot.

Computes exact differential analytics between two JMeter test runs:
...
def get_available_runs() -> List[Dict[str, Any]]:
    ...
    try:
        ...
        print(f"[compare_service] Error loading runs.json: {e}", flush=True)
        return []
...
def load_run_data(run_id: str) -> Optional[Dict[str, Any]]:
    ...
    if res_path.exists():
        try:
            ...
            print(f"[compare_service] Error reading {res_path}: {e}", flush=True)
...
def compare_two_runs(baseline_id: str, current_id: str) -> Dict[str, Any]:
    ...
    try:
        ...
        print(f"[compare_service] SLA load warning: {e}", flush=True)
    ...
    try:
        ...
        print(f"[compare_service] Hierarchy parse warning: {hier_err}", flush=True)
...
def _generate_comparative_ai_insights(sc: Dict[str, Any], txs: List[Dict[str, Any]],
    ...
    try:
        from python_files.ai_insights import generate_2run_comparison_ai_insights
        ...
        print(f"[compare_service] Live LLM generation warning: {e}.", flush=True)
    ...
    try:
        ...
        if res_path.exists():
            try:
                ...
                print(f"[compare_service] Result JSON draft sync note: {r_err}", flush=True)

        return {"success": True, "message": "Comparison draft saved successfully", "draft": draft_record}
    except Exception as e:
        print(f"[compare_service] Error saving draft: {e}", flush=True)
...
def load_comparison_draft(current_id: str) -> Optional[Dict[str, Any]]:
    ...
    if draft_file.exists():
        try:
            ...
            print(f"[compare_service] Error reading draft file {draft_file}: {e}", flush=True)
    ...
    for d in (_RESULTS_JSON_DIR, _RESULTS_DIR):
        ...
        if draft_file.exists():
            try:
                ...
                print(f"[compare_service] Error unlinking {draft_file}: {e}", flush=True)
