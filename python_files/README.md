...
The backend follows a **modular functional pipeline**. Test execution runs independently to generate raw JTL logs, which are processed by the dedicated `jtl_parser.py` engine before flowing into analytics, rule engines, AI insights, and interactive HTML reports.
...
flowchart TD
    ...
    subgraph Reporting ["5. Output & Historical Analysis"]
    ...
    JTLParser -->|Parsed Metrics & Timeseries| Apdex
    JTLParser -->|Client Metrics| Correlation
    AzureMon -->|Server Metrics| Correlation

    JTLParser -->|Parsed Metrics| Findings
    AzureMon -->|Server Telemetry| Findings
    
    Correlation -->|Bottleneck Context| AI
    Findings -->|Issues & Grades| AI
    JTLParser -->|Test Summary| AI
    ...
    JTLParser -->|Performance Metrics| ReportGen
...
| [`report_generator.py`](#3-report_generatorpy) | Reporting | Generates self-contained interactive HTML dashboards with Chart.js charts and analytics |
| [`ai_insights.py`](#4-ai_insightspy) | Intelligence | Generates diagnostic insights, root-cause deductions, and performance scores via LLMs / rules |
| [`findings_engine.py`](#5-findings_enginepy) | Analytics | Evaluates statistical performance thresholds, chart observations, and automated recommendations |
| [`correlation_engine.py`](#6-correlation_enginepy) | Analytics | Cross-correlates client-side latency/TPS with server-side CPU/Memory/IOPS |
| [`sla_manager.py`](#7-sla_managerpy) | Configuration | Parses JMX hierarchies (Thread Groups, Samplers) and manages SLA threshold targets |
| [`jmx_editor.py`](#8-jmx_editorpy) | Configuration | Safely reads and updates user concurrency, ramp-up, and duration inside `.jmx` XML scripts |
| [`apdex_calculator.py`](#9-apdex_calculatorpy) | Analytics | Computes Apdex user satisfaction scores and ratings based on target thresholds ($T$ and $4T$) |
| [`azure_monitor.py`](#10-azure_monitorpy) | Telemetry | Collects server-side telemetry from Azure Monitor API or realistic local mock metrics |
| [`trend_engine.py`](#11-trend_enginepy) | Analytics | Analyzes multi-run historical trends, health scores, and generates side-by-side run comparisons |
| [`organize_results.py`](#12-organize_resultspy) | Utility | Migrates and organizes loose test outputs into structured timestamped run folders |
| [`recompile_latest.py`](#13-recompile_latestpy) | Utility | Recompiles the HTML report for the most recent run without re-running JMeter |
| [`recompile_all.py`](#14-recompile_allpy) | Utility | Batch re-parses JTLs and regenerates HTML reports for all past test runs |

---

### 1. `run_local_jmeter.py`
**Test Orchestrator & Execution Engine**
- **Functional Purpose:** Discovers the local Apache JMeter installation, launches the non-GUI test execution, tails `.jtl` output in real time for live UI feedback, and produces the raw `.jtl` execution log.
- **Inputs:**
...
- **Outputs:**
  - Raw execution log (`.jtl` file) and runtime console logs (`jmeter.log`).
  - Run metadata (start/end timestamps, exit codes).

---

### 2. `jtl_parser.py`
**JMeter JTL / CSV Results Parser Engine**
- **Functional Purpose:** Dedicated parser that reads raw JMeter CSV logs (`.jtl`), computes statistical response time percentiles (p50, p90, p95, p99), aggregates transaction-level summaries, extracts error diagnostics (HTTP status codes, messages), and creates time-bucketed series for chart visualizations.
- **Inputs:**
  - `jtl_path`: Path to the raw JMeter `.jtl` result file.
- **Outputs:**
  - `parsed` dictionary containing:
    - `summary`: Aggregate metrics (total requests, avg RT, percentiles, throughput, error rates).
    - `labels`: Per-transaction metrics (counts, latencies, percentiles, error rates).
    - `time_series`: Dynamic 10s/60s bucketed arrays for Chart.js (RT, TPS, errors).
    - `error_details`: Categorized error codes, failure messages, and occurrence samples.
    - `raw_rows`: Total parsed sample count.
...
- **Inputs:**
  - `parsed`: Test summary, transaction-level metrics, error breakdown, and time-series data from `jtl_parser.py`.
