...
def generate_reportt(parsed: dict, azure_data: dict, ai_insights: dict,
...
<div class="report-container">
    ...
    <div class="tab-nav">
        ...
        # <button class="nav-btn" id="nav-btn-comparison" onclick="switchReportTab('rpt-compare', this)">⚖️ Run Comparison</button>
    </div>
    ...
    <div id="rpt-compare" class="tab-pane hidden">
        
        <!-- Comparison Selector Bar (Editable Mode Only) -->
        <div id="compare-selector-bar" class="section glass-panel" style="margin-bottom:1.5rem; border:1px solid rgba(56, 189, 248, 0.35); background: linear-gradient(135deg, rgba(56, 189, 248, 0.05), rgba(139, 92, 246, 0.05));">
            <div style="display:flex; justify-content:space-between; align-items:center; flex-wrap:wrap; gap:1rem;">
                <div>
                    <h2 style="margin:0; font-size:1.25rem; color:var(--text); display:flex; align-items:center; gap:0.5rem;">
                    ...
                    <p style="margin:0.25rem 0 0 0; color:var(--muted); font-size:0.85rem;">
                        Select a prior test execution to calculate exact differential KPIs, regression status, and AI insights.
                    </p>
                </div>
                <div style="display:flex; align-items:center; gap:0.75rem; flex-wrap:wrap;">
                    <div style="display:flex; align-items:center; gap:0.4rem;">
                        <label style="font-size:0.85rem; font-weight:600; color:var(--muted);">Current Run:</label>
                        <span class="status-pill" style="background:var(--accent); color:#0f172a; font-weight:700; font-size:0.8rem;">{run_id} ({users} Users)</span>
                    </div>
                    <span style="font-weight:700; color:var(--muted); font-size:0.9rem;">VS</span>
                    <div style="display:flex; align-items:center; gap:0.4rem;">
                        <label style="font-size:0.85rem; font-weight:600; color:var(--muted);">Baseline Run:</label>
                        <select id="compareBaselineSelect" onchange="onBaselineSelectionChange()" style="padding:0.45rem 0.85rem; border-radius:6px; border:1px solid var(--border); background:var(--surface2); color:var(--text); font-size:0.85rem; min-width:220px; font-family:inherit; outline:none;">
                            <option value="">-- Choose a Baseline Run --</option>
                        </select>
                    </div>
                    <button id="clearCompareBtn" class="btn" onclick="clearReportComparison()" style="background:rgba(239, 68, 68, 0.1); border-color:rgba(239, 68, 68, 0.3); color:#ef4444; display:none; padding:0.45rem 0.85rem;">✕ Clear</button>
                </div>
            </div>
        </div>
        ...
        <div id="compare-empty-state" class="section glass-panel text-center" style="padding:3.5rem 2rem; border-style:dashed;">
            ...
            <p style="max-width:560px; margin:0 auto 1.5rem auto; color:var(--muted); font-size:0.92rem; line-height:1.6;">
                Choose any previous test run from the dropdown above to generate an immediate side-by-side comparison including <strong>AI differential insights</strong>, <strong>iteration volume</strong>, <strong>response time divergence</strong>, <strong>error shifts</strong>, and <strong>critical SLA transitions</strong>.
            </p>
            <div style="display:inline-flex; gap:0.5rem; flex-wrap:wrap; justify-content:center;">
                <span class="status-pill" style="background:var(--surface2); color:var(--muted);">🤖 AI Insights</span>
                <span class="status-pill" style="background:var(--surface2); color:var(--muted);">📋 Iteration Stats</span>
                <span class="status-pill" style="background:var(--surface2); color:var(--muted);">⏱️ RT Divergence</span>
                <span class="status-pill" style="background:var(--surface2); color:var(--muted);">🔴 Error Delta</span>
                <span class="status-pill" style="background:var(--surface2); color:var(--muted);">🎯 SLA Transitions</span>
            </div>
        </div>
        ...
        <div id="compare-content-container" style="display:none;">
            ...
            <!-- 1. AI Comparative Insights -->
            <div class="section glass-panel" style="border: 1px solid rgba(139, 92, 246, 0.3); background: linear-gradient(135deg, rgba(139, 92, 246, 0.04), rgba(56, 189, 248, 0.04));">
                <div style="display:flex; justify-content:space-between; align-items:center; flex-wrap:wrap; gap:0.75rem; margin-bottom:1rem;">
                    <h2 style="margin:0; font-size:1.15rem; display:flex; align-items:center; gap:0.5rem;">
                        <span>🤖 AI Comparative Insights &amp; Verdict</span>
                    </h2>
                    <div id="comp-ai-risk-badge" class="status-pill" style="background:var(--green); color:#fff; font-weight:700;">LOW RISK</div>
                </div>
                <div id="comp-ai-summary" style="font-size:0.95rem; line-height:1.6; color:var(--text); margin-bottom:1rem;" contenteditable="true">
                    Loading comparative summary...
                </div>
                <div style="display:grid; grid-template-columns: repeat(auto-fit, minmax(280px, 1fr)); gap:1rem; margin-top:1rem;">
                    <div style="background:var(--surface2); border:1px solid var(--border); border-radius:8px; padding:1rem;">
                        <div style="font-size:0.85rem; font-weight:700; color:var(--text); margin-bottom:0.5rem;">⚡ Key Highlights &amp; Shifts</div>
                        <ul id="comp-ai-highlights" style="margin:0; padding-left:1.2rem; font-size:0.88rem; color:var(--muted); line-height:1.5;">
                            <li>Loading highlights...</li>
                        </ul>
                    </div>
                    <div style="background:var(--surface2); border:1px solid var(--border); border-radius:8px; padding:1rem;">
                        <div style="font-size:0.85rem; font-weight:700; color:var(--text); margin-bottom:0.5rem;">🛠️ Recommended Actions</div>
                        <ul id="comp-ai-recommendations" style="margin:0; padding-left:1.2rem; font-size:0.88rem; color:var(--muted); line-height:1.5;">
            ...
            <!-- 2. Iteration & Throughput Stats -->
            <div class="section glass-panel">
                <h2>📋 Iteration &amp; Throughput Stats Comparison</h2>
                ...
                <div style="display:grid; grid-template-columns: repeat(auto-fit, minmax(320px, 1fr)); gap:1.5rem; margin-top:1rem;">
                    <div class="chart-box" style="height:320px;">
                        <h4 style="margin:0 0 0.5rem 0; font-size:0.88rem; color:var(--muted);">Throughput (TPS) Comparison</h4>
                    ...
                    <div style="max-height:320px; overflow-y:auto; border:1px solid var(--border); border-radius:8px;">
                        <table style="width:100%; font-size:0.82rem;">
                            <thead>
                                <tr>
                                    <th>Transaction</th>
                                    <th style="text-align:right;">Baseline TPS</th>
                                    <th style="text-align:right;">Current TPS</th>
                                    <th style="text-align:right;">TPS Δ %</th>
                                </tr>
                            </thead>
                            <tbody id="comp-tps-tbody">
                                <tr><td colspan="4" class="text-center text-muted">Loading throughput...</td></tr>
                            </tbody>
                        </table>
                    </div>
                </div>
            ...
            <!-- 3. Response Time Stats & Hero Diverging Variance Chart -->
            <div class="section glass-panel">
                <h2>⏱️ Response Time Comparison &amp; Variance</h2>
                <div class="comp-kpi-grid">
                    <div class="comp-kpi-card">
                    ...
                    <div class="comp-kpi-card">
                        <div class="comp-kpi-title">99th Percentile (P99)</div>
                        <div class="comp-kpi-split">
                            <span class="comp-kpi-prev" id="comp-kpi-p99-a">-</span>
                            <span class="comp-kpi-arrow">➔</span>
                            <span id="comp-kpi-p99-b">-</span>
                        </div>
                        <div id="comp-badge-p99" class="comp-delta-badge neutral">Tail Latency</div>
                    </div>
                </div>

                <!-- Hero Diverging Bar Chart -->
                <div class="chart-box" style="margin-bottom:1.5rem; height:340px;">
                    <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:0.5rem;">
                        <h4 style="margin:0; font-size:0.9rem; color:var(--text);">📉 Response Time % Change (Hero Diverging Chart)</h4>
                        <div style="font-size:0.75rem; font-weight:700; display:flex; gap:0.75rem;">
                            <span style="color:var(--green);">← Improved (&lt; 0%)</span>
                            <span style="color:var(--red);">Degraded (&gt; 0%) →</span>
                        </div>
                    </div>
                ...
                <div style="max-height:420px; overflow-y:auto; border:1px solid var(--border); border-radius:8px;">
                    <table style="width:100%; font-size:0.83rem;">
                        ...
                        <tbody id="comp-rt-tbody">
                            <tr><td colspan="9" class="text-center text-muted">Loading transaction response times...</td></tr>
                        </tbody>
                    </table>
                </div>
            </div>

            <!-- 4. Error & Reliability Comparison -->
            <div class="section glass-panel">
                <h2>🔴 Error &amp; Reliability Comparison</h2>
                <div class="comp-kpi-grid">
                    <div class="comp-kpi-card">
                ...
                <div style="display:grid; grid-template-columns: repeat(auto-fit, minmax(320px, 1fr)); gap:1.5rem; margin-top:1rem;">
                    <div class="chart-box" style="height:280px;">
                        <h4 style="margin:0 0 0.5rem 0; font-size:0.88rem; color:var(--muted);">Error Rate (%) per Transaction</h4>
                        <canvas id="chart-comp-errors"></canvas>
                    </div>
                    <div style="max-height:280px; overflow-y:auto; border:1px solid var(--border); border-radius:8px;">
                        <table style="width:100%; font-size:0.82rem;">
                            <thead>
                                <tr>
                                    <th>Transaction</th>
                                    <th style="text-align:right;">Baseline Err %</th>
                                    <th style="text-align:right;">Current Err %</th>
                                    <th style="text-align:right;">Shift (pp)</th>
                                </tr>
                            </thead>
                            <tbody id="comp-err-tbody">
                                <tr><td colspan="4" class="text-center text-muted">Loading errors...</td></tr>
                            </tbody>
                        </table>
                    </div>
                </div>
            ...
            <!-- 5. Critical Transactions & SLA Transition Matrix -->
            <div class="section glass-panel">
                <h2>🎯 Critical Transactions &amp; SLA Transition Matrix</h2>
                <div class="comp-matrix-grid">
                    <div class="comp-matrix-box pass-pass">
                        <div style="font-size:0.8rem; font-weight:700; color:var(--green);">🟢 Pass ➔ Pass</div>
                        <div style="font-size:1.5rem; font-weight:800; color:var(--text);" id="comp-cnt-pass-pass">0</div>
                        <div style="font-size:0.75rem; color:var(--muted);">Consistently Met SLA</div>
                    </div>
                    <div class="comp-matrix-box pass-fail">
                        <div style="font-size:0.8rem; font-weight:700; color:var(--red);">🔴 Pass ➔ Fail (NEW BREACHES)</div>
                        <div style="font-size:1.5rem; font-weight:800; color:var(--red);" id="comp-cnt-pass-fail">0</div>
                        <div style="font-size:0.75rem; color:var(--red); font-weight:600;">Regressions Requiring Attention</div>
                    </div>
                    <div class="comp-matrix-box fail-pass">
                        <div style="font-size:0.8rem; font-weight:700; color:var(--green);">🟢 Fail ➔ Pass (RESOLVED)</div>
                        <div style="font-size:1.5rem; font-weight:800; color:var(--green);" id="comp-cnt-fail-pass">0</div>
                        <div style="font-size:0.75rem; color:var(--muted);">Recovered within SLA</div>
                    </div>
                    <div class="comp-matrix-box fail-fail">
                        <div style="font-size:0.8rem; font-weight:700; color:var(--yellow);">🟠 Fail ➔ Fail (PERSISTENT)</div>
                        <div style="font-size:1.5rem; font-weight:800; color:var(--text);" id="comp-cnt-fail-fail">0</div>
                        <div style="font-size:0.75rem; color:var(--muted);">Continued SLA Violations</div>
                    </div>
                </div>

                <h3 style="font-size:1rem; margin:1rem 0 0.5rem 0;">Critical Business Transactions Evaluation</h3>
...
<script>
    ...
    /* ── Comparison Tab Management & Chart Renderers ── */
    let compChartVariance = null;
    let compChartTps = null;
    let compChartErr = null;
    ...
    async function initComparisonTab() {{
        ...
        // In published mode, skip fetching runs
    ...
    async function loadReportComparison(baselineId) {{
        const currentRunId = "{run_id}";
        ...
        try {{
            ...
            // 1. AI Insights & Verdict
            const aiRiskBadge = document.getElementById('comp-ai-risk-badge');
            if (aiRiskBadge) {{
                aiRiskBadge.textContent = ai.risk_level || 'LOW RISK';
                aiRiskBadge.style.background = ai.risk_color || 'var(--green)';
            }}
            const aiSummary = document.getElementById('comp-ai-summary');
            if (aiSummary) aiSummary.innerHTML = ai.executive_summary || 'Comparative analysis completed.';

            const aiHighlights = document.getElementById('comp-ai-highlights');
            if (aiHighlights) {{
                aiHighlights.innerHTML = (ai.highlights && ai.highlights.length)
                    ? ai.highlights.map(h => `<li>${{h}}</li>`).join('')
                    : '<li>No significant regressions or deviations identified.</li>';
            }}

            const aiRecs = document.getElementById('comp-ai-recommendations');
            if (aiRecs) {{
                aiRecs.innerHTML = (ai.recommendations && ai.recommendations.length)
                    ? ai.recommendations.map(r => `<li>${{r}}</li>`).join('')
                    : '<li>System stability and performance SLA criteria are met across executions.</li>';
            }}
            ...
            if (rtTbody) {{
                rtTbody.innerHTML = txs.map(t => {{
                    ...
                    const diffSign = t.rt_diff_ms > 0 ? '+' : '';
                    const pctSign = t.rt_pct_change > 0 ? '+' : '';
                    return `
                        <tr>
            ...
            // B. Throughput Table
            const tpsTbody = document.getElementById('comp-tps-tbody');
            if (tpsTbody) {{
                tpsTbody.innerHTML = txs.map(t => `
                    <tr>
                        <td style="font-weight:600;">${{t.label}}</td>
                        <td style="text-align:right;">${{t.tps_a}}</td>
                        <td style="text-align:right; font-weight:700;">${{t.tps_b}}</td>
                        <td style="text-align:right; color:${{t.tps_pct_change >= 0 ? 'var(--green)' : 'var(--red)'}};">${{t.tps_pct_change >= 0 ? '+' : ''}}${{t.tps_pct_change}}%</td>
                    </tr>
                `).join('');
            }}

            // C. Error Table
            const errTbody = document.getElementById('comp-err-tbody');
            if (errTbody) {{
                errTbody.innerHTML = txs.map(t => `
                    <tr>
                        <td style="font-weight:600;">${{t.label}}</td>
                        <td style="text-align:right;">${{t.err_rate_a}}%</td>
                        <td style="text-align:right; font-weight:700; color:${{t.err_rate_b > 0 ? 'var(--red)' : 'var(--green)'}};">${{t.err_rate_b}}%</td>
                        <td style="text-align:right; color:${{t.err_change_pp > 0 ? 'var(--red)' : 'var(--green)'}};">${{t.err_change_pp > 0 ? '+' : ''}}${{t.err_change_pp}} pp</td>
                    </tr>
                `).join('');
            }}

            // D. Critical Transactions Table
            const critTbody = document.getElementById('comp-critical-tbody');
            if (critTbody) {{
                const targetCrit = critTxs.length ? critTxs : txs.slice(0, 10);
                critTbody.innerHTML = targetCrit.map(t => {{
    ...
    function renderComparisonCharts(txs) {{
        if (typeof Chart === 'undefined') return;

        // 1. Diverging Response Time % Variance Chart (Horizontal)
        const canvasVariance = document.getElementById('chart-comp-variance');
        if (canvasVariance) {{
            if (compChartVariance) compChartVariance.destroy();

            // Sort by largest absolute % change, take top 18
            const sortedForChart = [...txs].sort((a, b) => Math.abs(b.rt_pct_change) - Math.abs(a.rt_pct_change)).slice(0, 18);
            const labels = sortedForChart.map(t => t.label.length > 28 ? t.label.substring(0, 26) + '...' : t.label);
            const dataValues = sortedForChart.map(t => t.rt_pct_change);
            const bgColors = dataValues.map(v => v > 0 ? 'rgba(239, 68, 68, 0.75)' : 'rgba(16, 185, 129, 0.75)');
            const borderColors = dataValues.map(v => v > 0 ? '#dc2626' : '#059669');

            compChartVariance = new Chart(canvasVariance, {{
                type: 'bar',
                data: {{
                    labels: labels,
                    datasets: [{{
                        label: 'Response Time Change %',
                        data: dataValues,
        ...
        const canvasTps = document.getElementById('chart-comp-throughput');
        if (canvasTps) {{
            if (compChartTps) compChartTps.destroy();
            const topTxs = txs.slice(0, 12);
            const labels = topTxs.map(t => t.label.length > 22 ? t.label.substring(0, 20) + '...' : t.label);

            compChartTps = new Chart(canvasTps, {{
                type: 'bar',
                data: {{
                    labels: labels,
                    datasets: [
        ...
        // 3. Error Comparison Chart
        const canvasErr = document.getElementById('chart-comp-errors');
        if (canvasErr) {{
            if (compChartErr) compChartErr.destroy();
            const topTxs = txs.slice(0, 12);
            const labels = topTxs.map(t => t.label.length > 22 ? t.label.substring(0, 20) + '...' : t.label);

            compChartErr = new Chart(canvasErr, {{
                type: 'bar',
                data: {{
                    labels: labels,
                    datasets: [
                        {{
                            ...
                            data: topTxs.map(t => t.err_rate_a),
                        ...
                        {{
                            label: 'Current Error %',
                            data: topTxs.map(t => t.err_rate_b),
    ...
    function clearReportComparison() {{
        if (compChartVariance) {{ compChartVariance.destroy(); compChartVariance = null; }}
        if (compChartTps) {{ compChartTps.destroy(); compChartTps = null; }}
        if (compChartErr) {{ compChartErr.destroy(); compChartErr = null; }}
        ...
        if (contentContainer) {{ contentContainer.style.display = 'none'; contentContainer.innerHTML = ''; }}
