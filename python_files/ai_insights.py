...
def _log_ai_prompt(provider: str, model: str, prompt: str, action: str = "insights") -> None:
    ...
    try:
        ...
        # 1. Write full un-truncated prompt
        header = (
            f"{'='*80}\n"
            f"TIMESTAMP:     {ts}\n"
            f"PROVIDER:      {provider}\n"
...
def _log_ai_response(provider: str, model: str, raw_text: str, elapsed_ms: int,
    ...
    try:
        ...
        # 1. Write full un-truncated raw response
        header = (
            f"{'='*80}\n"
            f"TIMESTAMP:       {ts}\n"
            f"PROVIDER:        {provider}\n"
...
def _pure_python_repair_json(s: str) -> dict:
    ...
    # 2. Walk characters tracking quotes and escapes
    in_string = False
    escape = False
    clean_chars = []
    for ch in s:
    ...
    # 3. Strip dangling keys, colons, or commas at the truncation point
    for _ in range(5):
        res = res.strip()
        res = re.sub(r',\s*$', '', res)
    ...
    # 4. Trace bracket stack and close open delimiters
    in_str = False
    esc = False
    stack = []
    for ch in res:
...
def _safe_json_loads(text: str, provider_name: str = "AI") -> dict:
    ...
    # 2. Pure-Python repair (handles cut-off strings, open arrays/objects without third-party library)
    try:
    ...
    # 3. Third-party json_repair library if installed
    try:
        import json_repair
    ...
    # 4. Extract JSON object substring between outer braces and retry
    start = t.find("{")
    end = t.rfind("}") + 1
    if start >= 0:
    ...
    # Record error and raise clear exception
...
def execute_openrouter_prompt(prompt: str, api_key: str = None, model: str = "google/gemini-2.5-flash",
    ...
    # Default to 3000 max_tokens to prevent 402 credit overdrafts
    ...
    try:
        ...
        # Handle 402: "You requested up to X tokens, but can only afford Y"
        if err.code == 402:
...
def generate_2run_comparison_ai_insights(scorecard: dict, transactions: list, new_breaches: list,
    ...
    """Generates actual deep LLM insights comparing two JMeter test executions."""
    _load_env()
    preferred_provider = os.environ.get("DEFAULT_AI_PROVIDER", "").strip().lower()
    preferred_model = os.environ.get("DEFAULT_AI_MODEL", "").strip()
    openrouter_key = os.environ.get("OPENROUTER_API_KEY", "").strip()
    ...
    from python_files.ai_prompts import build_2run_comparison_prompt
...
def execute_chat_completion(system_prompt: str, messages: list, temperature: float = 0.2) -> tuple[str, str, int]:
    ...
    if (pref_provider == "openrouter" or not pref_provider) and openrouter_key:
        ...
        # Add active fast fallback models on OpenRouter
        fallbacks = [
            "openrouter/free",
            "google/gemma-4-31b-it:free",
            "liquid/lfm-2.5-2.6b:free",
