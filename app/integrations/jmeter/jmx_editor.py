...
def read_jmx_config(jmx_path: Path) -> dict:
            ...
            if prop_name == "ThreadGroup.num_threads":
                ...
                for subchild in child:
                    ...
                    if sub_name == "LoopController.loops":
                        try:
                            loops = int(subchild.text or "1")
                            config["loop_count"] = loops
                            config["iterations"] = loops
                        except ValueError:
                            config["loop_count"] = 1
                            config["iterations"] = 1
    ...
    # Extract all Sampler labels (transaction names) and TransactionControllers
    labels = set()
    transactions = set()
    for elem in root.iter():
        if elem.tag.endswith("Sampler") or elem.tag == "HTTPSamplerProxy":
    ...
    # Extract CSV Data Set Config filenames and check existence
    csv_files = []
    missing_csv_count = 0
    for elem in root.iter():
        if elem.tag == "CSVDataSet":
            csv_name = elem.attrib.get("testname", "CSV Data Set Config")
            filename_val = ""
            for child in elem:
                if child.attrib.get("name") == "filename":
                    filename_val = (child.text or "").strip()
                    break

            # Check if file exists relative to Tests/ or absolute
            base_csv_name = ""
            exists = False
            if filename_val:
                raw_p = Path(filename_val.replace("\\", "/"))
                base_csv_name = raw_p.name
                target_f = _TESTS_DIR / base_csv_name
                if target_f.exists():
                    exists = True
                elif raw_p.exists():
                    exists = True
