#!/usr/bin/env python3
"""
prompts.py — Standardized Prompt Engineering & Templates for PerfPilot AI Engine.
...
def _format_azure_infra_telemetry(infra: dict) -> str:
    ...
    avg_mem = infra.get("avg_memory", 0)
    max_mem = infra.get("max_memory", 0)
    net_in = infra.get("avg_network_in_mbps", 0)
    net_out = infra.get("avg_network_out_mbps", 0)

    lines.append(f"  Host Compute: Avg CPU={avg_cpu:.1f}%, Peak CPU={max_cpu:.1f}%, Avg Memory={avg_mem:.1f}%, Peak Memory={max_mem:.1f}%")
    lines.append(f"  Network I/O: Ingress={net_in:.2f} MB, Egress={net_out:.2f} MB")

    app_svc = infra.get("app_service", {})
    if app_svc:
        lines.append(f"  App Service: HTTP 5xx Server Errors={app_svc.get('http_5xx', 0)}, HTTP 2xx={app_svc.get('http_2xx', 0)}, Avg App Latency={app_svc.get('avg_response_time', 0):.2f}s")

    aks = infra.get("aks", {})
    if aks:
        lines.append(f"  AKS Cluster: Pod Ready Ratio={aks.get('pod_ready_pct', 0):.1f}%, Node CPU={aks.get('node_cpu_pct', 0):.1f}%, Node Memory={aks.get('node_memory_pct', 0):.1f}%")

    sql = infra.get("sql", {})
    if sql:
        lines.append(f"  SQL Database: DTU/CPU %={sql.get('cpu_pct', 0):.1f}%, Connection Count={sql.get('connections', 0)}, Deadlocks={sql.get('deadlocks', 0)}")

    return "\n".join(lines) if lines else "  Server telemetry empty"


def _format_error_breakdown(error_details: dict) -> str:
    """Format categorized error table with response codes, messages, and impacted samplers."""
