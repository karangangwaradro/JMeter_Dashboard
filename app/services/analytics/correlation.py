#!/usr/bin/env python3
"""
correlation.py — Client ↔ Server Metric Correlation.

Correlates client-side test metrics (response times, throughput, errors)
with server-side metrics (CPU, memory, network) to identify
...
def correlate_metrics(parsed: dict, azure_data: dict) -> dict:
    """
    Correlate client-side metrics with server-side monitoring data.
    ...
    if ts_cpu and ts_avg_rt:
        ...
        if high_cpu_high_rt > min_len * 0.3:
            findings.append({
